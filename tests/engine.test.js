...
out2.push('=== 23. 简历排版风格样本（第二十二轮） ===');
{
  ...
  /* --- 23.10 第五十二轮：要点中途的裸行是续写，不是新经历（京东健康假成就修复） --- */
  const frag = '设计曝光 25%／深度参与 45%／转化 30% 加权模型，压低转化权重以规避越权投放风险';
  const fixG = [
    '工作经历',
    '京东健康 数据产品运营实习生 2025.11 - 2026.01',
    '・负责商详页内容运营，页面转化率提升 15%',
    frag,
    '・对接数据团队输出周报'
  ].join('\n');
  const g1 = SRB3 && SRB3(fixG);
  ok3('夹具G：要点中途的裸行（含%，无符号）不单拎成新经历 → 仍 1 条', g1 && g1.length === 1, g1 && g1.length);
  ok3('夹具G：裸行内容并入京东健康的动作描述', g1 && g1.length === 1 && g1[0].action.indexOf('设计曝光 25%') >= 0,
    g1 && g1.length && g1[0].action.slice(0, 40));

  const fixH = fixG.split(frag).join('设计曝光 25%／深度参与 45%／转化 30% 加权模型：压低转化权重以规避越权投放风险');
  const h1 = SRB3 && SRB3(fixH);
  ok3('夹具H：裸行带冒号同样收编（「名称：描述」单行约定只对封口块生效）→ 仍 1 条', h1 && h1.length === 1, h1 && h1.length);

  const fixI = [
    '工作经历',
    '京东健康 数据产品运营实习生 2025.11 - 2026.01',
    '- 负责商详页内容运营',
    '- 设计曝光 25%／深度参与 45%／转化 30% 加权模型，压低转化权重'
  ].join('\n');
  const i1 = SRB3 && SRB3(fixI);
  ok3('夹具I：连字符要点（- 开头）是要点不是新经历 → 1 条且动作齐全', i1 && i1.length === 1 && i1[0].action.indexOf('加权模型') >= 0,
    i1 && i1.length && i1[0].action.slice(0, 40));
  ...
  const j1 = SRB3 && SRB3(fixJ);
  ok3('夹具J：表头后的首条裸行进动作，不再被胶进经历名也不单拎',
    j1 && j1.length === 1 && j1[0].project.indexOf('设计曝光') < 0 && j1[0].action.indexOf('设计曝光 25%') >= 0,
    j1 && JSON.stringify(j1.map(function (x) { return x.project; })));

  const fixK = [
    '工作经历',
    '京东健康 数据产品运营实习生 2025.11 - 2026.01',
    '・负责商详页内容运营，页面转化率提升 15%',
    'ceRNA 网络分析项目：对膜蛋白-配体互作网络进行加权与模块挖掘，识别 hub 基因 12 个。'
  ].join('\n');
  const k1 = SRB3 && SRB3(fixK);
  ok3('回归守卫：要点块之后真正的「名称：描述」单行经历仍独立成条',
    k1 && k1.length === 2 && k1[1].project === 'ceRNA 网络分析项目',
    k1 && JSON.stringify(k1.map(function (x) { return x.project; })));
