...
表单条目按**内容锚点配对**而不是按顺序填，页面缺经历栏时会引导补块后再填；
遇到识别不了的私有组件（自研下拉框等），可以在扩展里粘贴一条**适配规则**告诉它怎么取值。
...
项目配有三套无头自测，合计 **810 项断言**，覆盖解析、评分、匹配、排序与界面交互。
...
剩下两套（界面交互 96 项、扩展采集 258 项）需要 `jsdom`：
...
作者逐条验收。为了确保"设计能被验证"，才有了上面那 826 项断言 ——
界面不会报错的算法问题，只能靠测试发现。
