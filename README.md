...
[在线体验](https://YOURNAME.github.io/zhiyin/) · [界面预览](#界面预览) · [设计取舍](#几个刻意的取舍) · [快速开始](#快速开始)

<!-- 发布后请替换上面链接里的 YOURNAME -->
...
**7. 面向大众的文案。**
界面上不出现"这一版""原型示意""本地引擎"这类开发者自我指涉。
外部用户不需要知道内部结构，只需要知道下一步点哪里。
...
项目配有三套无头自测，合计 **777 项断言**，覆盖解析、评分、匹配、排序与界面交互。

其中核心算法层（**454 项**）**零依赖**，克隆下来就能跑：

```bash
node tests/engine.test.js
```

剩下两套（界面交互 86 项、扩展采集 230 项）需要 `jsdom`：
...
作者逐条验收。为了确保"设计能被验证"，才有了上面那 777 项断言 ——
界面不会报错的算法问题，只能靠测试发现。
